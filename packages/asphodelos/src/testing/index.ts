...
/**
 * Runs a generator against the real filesystem, the way the CLI does, and answers with what it
 * produced.
 *
 * A failure rejects, so a test that only cares that generation worked needs no assertion at all —
 * and when it does fail, the report carries the generator's own error rather than
